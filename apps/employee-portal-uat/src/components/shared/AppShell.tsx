/**
 * UAT AppShell — identical to the production AppShell but renders the
 * UATBanner above the header so the role switcher is always accessible.
 */
...
import { UATBanner } from '../../uat/UATBanner'
...
function getNavItems(allRoles: UserRole[]): NavItem[] {
  ...
  if (canAccessReview(allRoles))  items.push({ path: '/review',   label: 'Review Queue' })
  if (canAccessAdmin(allRoles))   items.push({ path: '/admin',    label: 'Admin' })
  if (canAccessPayroll(allRoles)) items.push({ path: '/payroll',  label: 'Payroll' })
  return items
}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalXS }}>
                {navItems.map(item => (
                  <Button
                    ...
                    style={{ justifyContent: 'flex-start', fontWeight: currentTab === item.path ? 600 : 400 }}
